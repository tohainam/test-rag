...
import { filesApi, type ParentChunk } from '@/features/files/api/files.api';
import { getErrorMessage } from '@/shared/types';
import { ChunkMetadataDrawer } from './ChunkMetadataDrawer';

interface ParentChunksTableProps {
  fileId: string;
}

const PAGE_SIZE = 20;

export function ParentChunksTable({ fileId }: ParentChunksTableProps) {
  const [chunks, setChunks] = useState<ParentChunk[]>([]);
  const [loading, setLoading] = useState(false);
  const [page, setPage] = useState(1);
  const [total, setTotal] = useState(0);
  const [selectedChunk, setSelectedChunk] = useState<ParentChunk | null>(null);
  const [drawerOpened, setDrawerOpened] = useState(false);

  const loadChunks = async (currentPage: number) => {
    try {
      setLoading(true);
      const response = await filesApi.getParentChunks(fileId, currentPage, PAGE_SIZE);
      ...
      if (response.success) {
        ...
        notifications.show({
          title: 'Error',
          message: 'Failed to load parent chunks',
          color: 'red',
        });
      }
    } catch (error) {
      notifications.show({
        title: 'Error',
        message: getErrorMessage(error) || 'Failed to load parent chunks',
        color: 'red',
      });
    } finally {
      setLoading(false);
  ...
  const handleViewMetadata = (chunk: ParentChunk) => {
  ...
  return (
    <>
      <DataTable
        ...
        minHeight={200}
        noRecordsText="No parent chunks found"
        highlightOnHover
